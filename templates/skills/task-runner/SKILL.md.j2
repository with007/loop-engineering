...
1. **收到匹配的 notification（status=completed）**：
   ...
   - `VFY` → 读 `.loop-engineering/vfy-output-r$ROUND.md`（此时 VFY 已退出，文件完整）→ 删 phase 文件 → PASS 进 Step 5，FAIL 进重试
...
用 `Agent` 工具，`run_in_background: true`。子代理**不会自动继承** worktree CWD — 必须在 prompt 中显式 cd 到 `$AGENT_DIR`。
