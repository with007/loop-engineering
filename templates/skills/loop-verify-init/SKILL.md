...
`loop setup` 的 `ensure_claude_rules` 会按 marker 向 CLAUDE.md 追加规则模板。多次 setup 或项目类型变更后，可能出现不同章节表达重复意思的情况。

读 CLAUDE.md，逐节对比：

- 新追加的 marker 规则（`<!-- loop:rule:xxx -->`）与手写的旧章节是否存在语义重复
- 不同章节是否有实质上讲同一件事的内容（如"日志怎么查"出现在两处）

发现重复后，向用户展示重叠的章节和你的合并建议，让用户决定删哪个、保留哪个。不要自作主张删除。
...
### CLAUDE.md 规则检查
- ✅ 无重复 / ⚠️ 发现「打开 Unity 工程」与「调试与日志」都提到 Editor.log 路径，建议合并到后者
