...
零依赖，纯 stdlib。可 import 也可直接 CLI 调用。
由 loop setup 部署到 .claude/scripts/。

作为模块:
  from task_line import TaskLine, update_task, load_tasks, save_tasks

作为 CLI:
  python task_line.py <task_id> <status> [--project-root <dir>]
...
示例:
  python .claude/scripts/task_line.py 770ea8b5 "~" --project-root D:/work_pvp/loop-engineering
...
def main():
    ...
    if len(sys.argv) < 3:
        print("用法: task_line.py <task_id> <status> [--project-root <dir>] [--append-meta <text>] [--set-meta <text>] [--add-feedback <text>] [--assignee <name>]")
        sys.exit(1)
