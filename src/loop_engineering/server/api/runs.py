...
def list_runs(
    ...
    entries = query_runs(_project_root(), whoami=whoami, result=result, days=days, limit=limit)
    return {"runs": entries, "count": len(entries)}


@router.get("/pass-rate")
def pass_rate(days: int = Query(7)):
    from loop_engineering.runlog import get_pass_rate
    passed, total, rate = get_pass_rate(_project_root(), days=days)
