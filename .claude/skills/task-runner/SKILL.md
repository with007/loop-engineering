...
- **子代理只输出结果信号** — IMP/VFY 的最终输出只能是一行 `PASS` 或 `FAIL: <原因>`。完整报告写入任务子目录下的带轮次后缀文件（`.loop-engineering/tasks/<taskID>/imp-output-r{N}.md` / `vfy-output-r{N}.md`），由 `taskhelper.py run-done` 读取组装 commit message。禁止子代理在最终输出中回传报告全文，避免污染 task-runner 上下文
...
phase 文件 `.loop-engineering/tasks/<taskID>/phase` 记录当前进行中任务的阶段、子代理 ID 和轮次。格式：`STAGE:AGENT_ID:BRANCH:ROUND`（ROUND 从 1 开始，每次 VFY FAIL 后 IMP 重试时递增）。

```bash
PHASE_OUT=$(python .claude/scripts/taskhelper.py get-phase --project-root $PROJECT_ROOT 2>/dev/null)
if echo "$PHASE_OUT" | grep -q "^PHASE="; then
  PHASE_STR=$(echo "$PHASE_OUT" | sed 's/^PHASE=\(.*\) TASK_ID=.*/\1/')
  TASK_ID=$(echo "$PHASE_OUT" | sed 's/^.*TASK_ID=//')
  IFS=':' read -r STAGE AGENT_ID BRANCH ROUND <<< "$PHASE_STR"
  ROUND="${ROUND:-1}"
  echo "PHASE=$STAGE AGENT=$AGENT_ID BRANCH=$BRANCH ROUND=$ROUND TASK_ID=$TASK_ID"
else
  TASK_ID=""
  echo "NO_PHASE"
fi
...
> state.json 为 phase 的权威来源，不再使用独立 phase 文件。
...
1. **收到匹配的 notification（status=completed）**：
   ...
   - `VFY` → 先检查 `.loop-engineering/tasks/$TASK_ID/vfy-output-r$ROUND.md` 是否存在：`test -f .loop-engineering/tasks/$TASK_ID/vfy-output-r$ROUND.md || { echo "vfy-output-r$ROUND.md 不存在，重试 VFY" && exit 1; }` → 存在则读文件判断 PASS/FAIL → 删 phase 文件 → PASS 进 Step 5，FAIL 进重试

2. **收到匹配的 notification（status=killed）**：
   - 重新派发同类型 agent（不删 phase 文件，更新 agent ID）：`python .claude/scripts/taskhelper.py phase "$TASK_ID" "$STAGE:$NEW_AGENT_ID:$BRANCH:$ROUND" --project-root $PROJECT_ROOT`
...
- `BUSY` → **跳过所有 git 操作**，直接进入 Step 1（taskhelper pick 会返回 BUSY，task-runner 等待）
- `IDLE` → 安全清理：
...
```bash
eval $(python .claude/scripts/project_vars.py) && whoami=$(python -c "import yaml; print(yaml.safe_load(open('.loop-engineering/loop-config.yaml', encoding='utf-8'))['agent']['name'])")
python .claude/scripts/taskhelper.py pick $whoami --project-root $PROJECT_ROOT
```
...
```bash
TASK_ID="<taskhelper pick 输出的 taskID= 字段>"
REOPEN="<taskhelper pick 输出的 reopen= 字段>"
if [ "$REOPEN" = "true" ]; then
  ROUND=$(python -c "
import glob, re
files = glob.glob('.loop-engineering/tasks/${TASK_ID}/imp-output-r*.md') + glob.glob('.loop-engineering/tasks/${TASK_ID}/vfy-output-r*.md')
...
> ROUND 从不重置。新任务 = 1，每次 VFY FAIL 递增，reopen 继续递增。`taskhelper.py run-start` 负责记录 start_round 到 state.json。

### Step 2: Fork 分支 + 标记进行中

```bash
eval $(python .claude/scripts/project_vars.py)
# 用 taskhelper pick 输出的完整分支名
BRANCH="<taskhelper pick 输出的 branch= 字段>"
REOPEN="<taskhelper pick 输出的 reopen= 字段>"
...
# 主工程 tasks.md 标记进行中（taskhelper run-start 同时更新 state.json + tasks.md）
# 记录 run 开始（state.json + tasks.md 状态→~）
python .claude/scripts/taskhelper.py run-start "$TASK_ID" --project-root "$PROJECT_ROOT"
```
...
|------|------|
| `imp \| vfy` | 子代理类型 |
| `--desc` | 任务描述（taskhelper pick 输出的 `desc=`） |
| `--task-id` | 任务 ID（taskhelper pick 输出的 `taskID=`） |
| `--branch` | 分支名（taskhelper pick 输出的 `branch=`） |
| `--round` | 当前轮次（默认 1） |
| `--user-feedback` | 用户反馈（taskhelper pick 输出的 `user_feedback=`） |
| `--open-spec` | OpenSpec 任务（taskhelper pick 输出的 `openSpec=true` 时传此 flag） |
| `--reopen` | 重开任务（taskhelper pick 输出的 `reopen=true` 时传此 flag，仅 imp） |
...
```bash
python .claude/scripts/taskhelper.py phase "$TASK_ID" "IMP:<IMP_AGENT_ID>:$BRANCH:$ROUND" --project-root $PROJECT_ROOT
```
...
1. IMP 已运行 **> 30 分钟**
2. 检查 `.loop-engineering/tasks/$TASK_ID/imp-output-r$ROUND.md` 是否持续为空或未更新（连续两次 `stat` 相同且无新内容）
...
```bash
python .claude/scripts/taskhelper.py phase "$TASK_ID" "VFY:<VFY_AGENT_ID>:$BRANCH:$ROUND" --project-root $PROJECT_ROOT
```
...
VFY notification 到达时文件一定完整。读 `.loop-engineering/tasks/$TASK_ID/vfy-output-r$ROUND.md` 判断结论：

```bash
python -c "
with open('.loop-engineering/tasks/${TASK_ID}/vfy-output-r$ROUND.md', encoding='utf-8') as f:
    for line in f:
        if '**PASS**' in line:
            print('PASS'); break
        elif '**FAIL**' in line:
...
- **FAIL** → task-runner 进入 FAIL 重试流程：
  ...
  2. SendMessage 当前 implementer，告知 VFY FAIL（IMP 自行读 `.loop-engineering/tasks/$TASK_ID/vfy-output-r*.md` 获取详情）
  3. implementer 修复，输出写入 `.loop-engineering/tasks/$TASK_ID/imp-output-r$ROUND.md`
  4. 重新派发 verifier（VFY 自行读取 `.loop-engineering/tasks/$TASK_ID/` 下历史 `vfy-output-r*.md`）
- **UNKNOWN** → 视为 FAIL
...
**PASS**:
1. 运行 `taskhelper.py run-done`（内部完成 state.json 更新 + tasks.md 同步 + git add/commit/push + diff）：
   ```bash
   python .claude/scripts/taskhelper.py run-done "$TASK_ID" --result pass --do-commit --project-root $PROJECT_ROOT
   ```
2. 清理 phase：
   ```bash
   python .claude/scripts/taskhelper.py phase "$TASK_ID" --clear --project-root $PROJECT_ROOT
   ```
3. 重置 working tree：
   ```bash
   eval $(python .claude/scripts/project_vars.py)
...
> **说明**：`taskhelper.py run-done --result pass --do-commit` 从 state.json 的 `runs[-1].outputs` 取输出文件列表，按交替格式组装 commit message，git add/commit/push，生成 diff，更新 state.json 和 tasks.md。task-runner 不读报告内容。
...
- 弹通知：
  ...
  ```
- 清理本地分支和任务目录 → `rm -rf .loop-engineering/tasks/$TASK_ID && git checkout --detach --force $DEFAULT_REF && git clean -fd && git branch -D <BRANCH>`
