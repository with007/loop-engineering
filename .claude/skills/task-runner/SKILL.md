...
- **禁止 `git checkout master`** — agent worktree 永远不能 checkout master（master 被主 worktree 占用）。只用 `HEAD` 远程引用。
- **同步用 `git fetch origin && git reset --hard HEAD`**（detached HEAD），或用 `git checkout -B agent/xxx HEAD`（fork 分支）。
- **禁止在 implementer 运行不到 30 分钟时主动 kill**
- **禁止在新起 implementer 时用 `checkout -B ... HEAD` 重置分支**（会丢弃 IMP 已做的工作）
...
# 删掉已合入 HEAD 的 agent 分支（不是全部删除）
git branch --list "agent/*" --merged HEAD | xargs -r git branch -d 2>/dev/null
```
...
**正常完成**: implementer 返回报告 → 进入 Step 4 派发 verifier。
...
1. IMP 已运行 **> 30 分钟**
2. 用 `TaskOutput(task_id, block=false)` 主动检查确认 **输出已停止**（连续两次检查 output 相同且无新内容）
3. 分支上 **无新 commit**：`git log <BRANCH> --not HEAD --oneline` 为空
...
- 弹通知：
  ...
  ```
- 清理本地分支 → `git checkout --detach HEAD && git branch -D <BRANCH>`
...
**审查**: git fetch && git diff HEAD...origin/<BRANCH>
```