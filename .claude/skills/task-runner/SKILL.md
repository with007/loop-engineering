...
- **子代理只输出结果信号** — IMP/VFY 的最终输出只能是一行 `PASS` 或 `FAIL: <原因>`。完整报告写入任务子目录下的带轮次后缀文件（`.loop-engineering/tasks/<taskID>/imp-output-r{N}.md` / `vfy-output-r{N}.md`），由 `task_done.py` 脚本读取组装 commit message。禁止子代理在最终输出中回传报告全文，避免污染 task-runner 上下文
...
```bash
PHASE_FILE=$(ls .loop-engineering/tasks/*/phase 2>/dev/null | head -1)

if [ -f "$PHASE_FILE" ]; then
  IFS=':' read -r STAGE AGENT_ID BRANCH ROUND <<< "$(cat $PHASE_FILE)"
  TASK_ID=$(echo "$PHASE_FILE" | sed 's|.*\.loop-engineering/tasks/||; s|/phase$||')
  # 兼容旧格式（无 ROUND 字段）
  ROUND="${ROUND:-1}"
...
2. **收到匹配的 notification（status=killed）**：
   - 重新派发同类型 agent（不删 phase 文件，更新 agent ID）：`echo "$STAGE:$NEW_AGENT_ID:$BRANCH:$ROUND" > .loop-engineering/tasks/$TASK_ID/phase`
...
```bash
TASK_ID="<task_pick 输出的 taskID= 字段>"
...
# 创建任务目录并记录起始轮次（task_done.py 只收集 >= start-round 的轮次）
mkdir -p ".loop-engineering/tasks/$TASK_ID"
echo $ROUND > ".loop-engineering/tasks/$TASK_ID/start-round"
```

> ROUND 从不重置。新任务 = 1，每次 VFY FAIL 递增，reopen 继续递增。`start-round` 标记本次执行的起始轮次，确保 commit 不重复包含历史轮次。
...
```bash
echo "IMP:<IMP_AGENT_ID>:$BRANCH:$ROUND" > .loop-engineering/tasks/$TASK_ID/phase
```
...
```bash
echo "VFY:<VFY_AGENT_ID>:$BRANCH:$ROUND" > .loop-engineering/tasks/$TASK_ID/phase
```
...
1. 运行 `task_done.py`（内部完成 git add/commit/push + diff + tasks.md + 通知）：
   ...
   python .claude/scripts/task_done.py $whoami [taskID] [ROUND] [ROUND] \
     ...
     --start-round $(cat .loop-engineering/tasks/$TASK_ID/start-round) \
     --do-commit
   ```
2. 清理 phase 文件 + working tree（保留输出文件以便 reopen 继续递增）：
   ```bash
   eval $(python .claude/scripts/project_vars.py)
   rm -f .loop-engineering/tasks/$TASK_ID/phase
   git checkout --detach --force $DEFAULT_REF && git clean -fd
   ```

> **说明**：`task_done.py --do-commit` 从 `.loop-engineering/tasks/<taskID>/` 收集 `>= --start-round` 的 `imp-output-r*.md` 和 `vfy-output-r*.md`，按交替格式（R{N}: IMP→VFY, R{N+1}: IMP→VFY, …）组装 commit message。只包含本次执行的轮次，不重复历史。然后 git add/commit/push，生成 diff、更新 tasks.md、弹通知。task-runner 不读报告内容。
...
- 弹通知：
  ...
  ```
- 清理本地分支和 phase 文件 → `rm -rf .loop-engineering/tasks/$TASK_ID && git checkout --detach --force $DEFAULT_REF && git clean -fd && git branch -D <BRANCH>`
