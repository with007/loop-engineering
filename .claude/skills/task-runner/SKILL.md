...
```
## 任务
<原样复制 tasks.md 中的任务描述，一字不改。不补充上下文、不分析需求>
...
用 `Agent` 工具，`run_in_background: true`，独立上下文，只能验证不能改代码。必须显式 cd 到 agent worktree。
...
Implementer 报告: 读 `.loop-engineering/imp-output.md` 了解实现细节。
...
4. 完成后将完整验证报告写入 `.loop-engineering/vfy-output.md`

## 输出

按以下格式输出（每节都会写入 git commit message，与人审查时快速理解变更全貌）：
...
- **PASS** → 进入 Step 5
- **FAIL** → 进入 FAIL 重试流程（不读报告内容，后续 SendMessage 时 verifier 自己会读）
