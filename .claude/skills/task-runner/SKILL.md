...
2. 清理 phase 文件 + working tree：
   ```bash
   eval $(python .claude/scripts/project_vars.py)
   rm -rf .loop-engineering/tasks/$TASK_ID
   git checkout --detach --force $DEFAULT_REF && git clean -fd
   ```

> **说明**：`task_done.py --do-commit` 会从 `.loop-engineering/tasks/<taskID>/` 收集所有 `imp-output-r*.md` 和 `vfy-output-r*.md`，按交替格式（R1: IMP→VFY, R2: IMP→VFY, …）组装 commit message。只有最后一轮保留完整报告，前轮 IMP 精简为"反馈 + 修复内容"。然后 git add/commit/push，生成 diff、更新 tasks.md、弹通知。task-runner 不读报告内容。
