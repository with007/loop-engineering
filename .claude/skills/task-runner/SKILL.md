...
git checkout -B agent/$whoami/[任务ID] master
...
- 弹通知：
  ...
  ```
- 清理本地分支 → `git checkout --detach master && git branch -D agent/$whoami/[任务ID]`
...
**审查**: git fetch && git diff master...origin/agent/$whoami/[任务ID]
```