...
- **每个任务从 master fork** — 分支 `agent/[用户名]/[任务ID]`，从最新 master 创建
- **透明管道** — task-runner 不参与任务具体实现。不研究代码、不分析需求、不补充上下文。原样传递 tasks.md 中的描述给 implementer，implementer 自行理解代码并设计方案
- **子代理只输出结果信号** — IMP/VFY 的最终输出只能是一行 `PASS` 或 `FAIL: <原因>`。完整报告只写入文件（`imp-output.md` / `vfy-output.md`），由 `task_done.py` 脚本读取组装 commit message。禁止子代理在最终输出中回传报告全文，避免污染 task-runner 上下文

## [WARNING] 关键禁令

- **禁止 `git checkout master`** — agent worktree 永远不能 checkout master（master 被主 worktree 占用）。只用 `master` 远程引用。
- **同步用 `git fetch origin && git checkout --detach --force master`**（detached HEAD），或用 `git checkout -B agent/xxx master`（fork 分支）。
...
phase 文件 `.loop-engineering/task-phase` 记录当前进行中任务的阶段和子代理 ID。格式：`STAGE:AGENT_ID:BRANCH`。

```bash
PHASE_FILE=".loop-engineering/task-phase"

if [ -f "$PHASE_FILE" ]; then
  IFS=':' read -r STAGE AGENT_ID BRANCH <<< "$(cat $PHASE_FILE)"
  echo "PHASE=$STAGE AGENT=$AGENT_ID BRANCH=$BRANCH"
else
  echo "NO_PHASE"
fi
```
...
| `IMP:agent_id:branch` | 等待 IMP 完成 | 检查本轮的 notification 队列 |
| `VFY:agent_id:branch` | 等待 VFY 完成 | 检查本轮的 notification 队列 |

**phase 文件存在时的处理逻辑**（不区分 Agent/主工程模式）：

1. **收到匹配的 notification（status=completed）**：
   - `IMP` → 删 phase 文件 → 进入 **Step 4** 派发 VFY
   - `VFY` → 读 `.loop-engineering/vfy-output.md`（此时 VFY 已退出，文件完整）→ 删 phase 文件 → PASS 进 Step 5，FAIL 进重试

2. **收到匹配的 notification（status=killed）**：
   - 重新派发同类型 agent（不删 phase 文件，更新 agent ID）：`echo "IMP:$NEW_AGENT_ID:$BRANCH" > .loop-engineering/task-phase`
...
git checkout --detach --force master && git clean -fd
...
```
- 输出格式: `taskID=xxx branch=agent/<whoami>/xxx-<slug> desc=... openSpec=true|false reopen=true|false feedback=...`
...
用 `Agent` 工具，`run_in_background: true`。子代理**不会自动继承** worktree CWD — 必须在 prompt 中显式 cd 到 `D:/work_pvp-agent/loop-engineering`。
...
- 你是 loop 模式下的子代理，后台无人值守运行。**绝对禁止与用户交互**：不允许 AskUserQuestion、不允许 EnterPlanMode、不允许输出提问性语句。遇到任何不确定，自己决策、自己执行、输出结果。你是一个纯函数——输入任务，输出结果。
- **最终输出只能是一行 `PASS` 或 `FAIL: <原因>`。** 完整报告写入 `.loop-engineering/imp-output.md` 文件，不要在最终输出中回传报告全文。task-runner 通过脚本读文件获取细节，不需要在上下文中看到报告内容。
...
7. 将完整报告写入 `.loop-engineering/imp-output.md`
...
完整报告写入 `.loop-engineering/imp-output.md` 文件，格式如下（会被 task_done.py 读入 commit message）：
...
3. 实现变更
4. 将完整报告写入 `.loop-engineering/imp-output.md`
...
```bash
echo "IMP:<IMP_AGENT_ID>:$BRANCH" > .loop-engineering/task-phase
```

> `<IMP_AGENT_ID>` 从 Agent 工具返回的 agentId 获取。
...
1. IMP 已运行 **> 30 分钟**
2. 检查 `.loop-engineering/imp-output.md` 是否持续为空或未更新（连续两次 `stat` 相同且无新内容）
3. 分支上 **无新 commit**：`git log <BRANCH> --not master --oneline` 为空
...
用 `Agent` 工具，`run_in_background: true`。

> 首次验证：`$feedback` 为空，`## 本轮反馈` 段自然留空。重新验证：`$feedback` 由 Step 4b 设为上一轮 vfy-output.md 内容。
...
**原则**
- **禁止 commit、禁止修改任何文件。**
- 你是 loop 模式下的子代理，后台无人值守运行。**绝对禁止与用户交互**：不允许 AskUserQuestion、不允许 EnterPlanMode、不允许输出提问性语句。遇到任何不确定，自己决策、自己执行。你是一个纯函数——输入任务，输出验证结果。
- **最终输出只能是一行 `PASS` 或 `FAIL: <原因>`。** 完整报告写入 `.loop-engineering/vfy-output.md` 文件，不要在最终输出中回传报告全文。task-runner 通过脚本读文件获取细节，不需要在上下文中看到报告内容。

**流程**
1. 读 `git diff`，对照任务描述理解变更
2. 按 `/loop-verify` 的方法论执行验证：表面识别 → 驱动 → 探测 → 报告
3. 将完整验证报告写入 `.loop-engineering/vfy-output.md`
4. 最后输出一行：`PASS` 或 `FAIL: <原因>`

## 输出

**⚠️ 你的最后一条消息只能是一行：`PASS` 或 `FAIL: <原因>`。** 完整报告已写入 `vfy-output.md`，报告格式遵循 `/loop-verify` 的规定。
```
...
```bash
echo "VFY:<VFY_AGENT_ID>:$BRANCH" > .loop-engineering/task-phase
```
...
VFY notification 到达时文件一定完整。读 `.loop-engineering/vfy-output.md` 判断结论：
...
- **PASS** → 进入 Step 5
- **FAIL** → task-runner 读完整 `.loop-engineering/vfy-output.md` 获取 FAIL 测试点，进入 FAIL 重试流程：
  1. SendMessage 当前 implementer，携带 FAIL 测试点
  2. implementer 修复
  3. 将上一轮 vfy-output.md 内容设为 `$feedback`，重新派发 verifier（写新的 VFY phase 文件）
- **UNKNOWN** → 视为 FAIL
...
- 弹通知：
  ...
  ```
- 清理本地分支和 phase 文件 → `rm -f .loop-engineering/task-phase && git checkout --detach --force master && git clean -fd && git branch -D <BRANCH>`
...
**审查**: git fetch && git diff master...origin/<BRANCH>
```