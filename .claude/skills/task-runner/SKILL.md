...
**派发前自检** — verifier prompt 必须包含以下 7 段（缺一不可，逐段核对后再调用 Agent）：

| # | 段落 | 来源 |
|---|------|------|
| 1 | 任务标识 | taskID + OpenSpec 路径，或 tasks.md 描述 |
| 2 | 工作目录 | 固定模板（含 bash 指令） |
| 3 | 变更 | 分支名 + implementer 完整报告 |
| 4 | **你的工作** | **必须包含 VERIFY.md 指令 + 工作步骤编号列表** |
| 5 | 输出格式 | 固定模板（验证方案/结果/局限） |
| 6 | 自主运行 | 固定模板（禁止交互） |
| 7 | retry 上下文 | 仅非首次验证时追加 |

> ⚠️ 第 4 段"你的工作"是最容易遗漏的段落。它包含 VERIFY.md 读取指令和 numbered list 工作步骤，**不可省略任何一条**。

---

**verifier prompt 公共尾部**（两个模式共用，直接追加在 prompt 末尾）：
...
**openSpec=true** 时，verifier prompt 头部 + 工作段落：
...
> 上述头部 + 工作段落后，**立即追加「公共尾部」**。

---

**openSpec=false** 时，verifier prompt 头部 + 工作段落：
...
5. 清理本地分支（仅推送成功后才删）：
   ...
   if git remote -v | grep -q origin; then
     ...
     echo "无 remote，保留分支 <BRANCH> 待手动合入 master"
   fi
   ```
