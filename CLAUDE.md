...
## 5. GUI/框架调试：先查框架限制，再怀疑代码

遇到 GUI 问题（窗口不显、事件不响应、循环卡死）时，**不要默认"我的代码有 bug"**。

1. 确认框架的设计假设与应用场景是否匹配。eframe 假设"始终有可见窗口要渲染"，托盘/后台应用不是它的设计目标。
2. 查框架官方 examples 里有没有类似场景的用法。`tray-icon/examples/winit.rs` 展示了正确的事件循环模式。
3. 如果改了 3 轮还没解决，**主动提出"可能选错了框架/API"，而不是继续猜测**。
4. 考虑用底层库替代封装层：eframe → winit + egui_winit 直接控制 `ControlFlow`。
