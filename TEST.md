...
关键页面：
- `/` — 仪表盘首页，展示项目统计（通过率图表、近期运行记录）。**操作**: 浏览器打开 `http://127.0.0.1:8765/`，确认图表渲染、数据加载正常
- `/tasks` — 任务列表，显示 tasks.md 中的任务状态和分配情况。**操作**: 点击导航栏"任务"或打开 `http://127.0.0.1:8765/tasks`，确认任务卡片正常显示、状态标签正确
- `/runs` — 运行历史，展示所有 IMP/VFY 运行记录。**操作**: 点击导航栏"运行记录"或打开 `http://127.0.0.1:8765/runs`，确认列表加载、筛选功能正常
- `/control` — 循环控制，管理 agent 循环的启停和心跳状态。**操作**: 点击导航栏"控制"或打开 `http://127.0.0.1:8765/control`，确认控制按钮可点击、状态信息正确
- `/settings` — 配置编辑，在线编辑 VERIFY.md 和 TEST.md。**操作**: 点击导航栏"设置"或打开 `http://127.0.0.1:8765/settings`，确认编辑器加载、保存功能正常
